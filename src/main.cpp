		...
		if (selection == "Edit file") {
			...
			while (true) {
				...
				// MINIMAL: avoid -1 on empty lines (prevents later replace() issues too)
				{
					...
					if (line_cursor.ch < h_offset)
						h_offset = line_cursor.ch;

					if (line_cursor.ch >= h_offset + width)
						h_offset = line_cursor.ch - width + 1;
				...
				for (int line = 0; line < real_height; line++) {
					...
					for (int ch = 0; ch < width; ch++) {
						int doc_ch = h_offset + ch;
						if (doc_ch >= (int)per_line[doc_line].length())
							break;
						...
						if (line_cursor.line == doc_line && line_cursor.ch == doc_ch) attron(COLOR_PAIR(color_pair));
						...
						printw("%c", per_line[doc_line][doc_ch]);
						if (line_cursor.line == doc_line && line_cursor.ch == doc_ch) attroff(COLOR_PAIR(color_pair));
				...
				while (temp_increment < (int)original_content.length()) {
				
				    if (temp_line == line_cursor.line &&
				        temp_ch == line_cursor.ch) {
				        break;
				    }
				...
				if (redact_state) {
				    original_content[temp_increment] = '^';
				    per_line[line_cursor.line][line_cursor.ch] = '^';
				}
			}
